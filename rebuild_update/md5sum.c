...
/*
 *  copy from http://www.netlib.org/crc/md5sum.c
 */
...
void md5sum(FILE *fd, char *name, int hex, char* md5str);
...
md5sum(FILE *fd, char *name, int hex, char* md5str)
{
	...
	if(hex){
        ...
        if( md5str )
        {
            for (i = 0; i<16; i++) md5str += sprintf(md5str, "%.2x", digest[i]);
        }
    }
    else{
